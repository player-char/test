...
let known = {
	...
	'спам|spam': (m) => {
		...
		while (mail.length < 950) {
			mail += chance(0.8) ? (chance(0.9) ? ' ' : '-') + spam : '. ? !'.spick() + (chance(0.2) ? '\n\n \n'.spick() : ' ') + slam;
		}
		return ['d', mail + '!'];
	},
...
let responses = [
	...
	*/
	...
	{
		d: true,
		p: /^ *(д?а )?([тв]ы )?ч((т|ег)?о|[её])( ([кт]ак))?[!?., ]*$/i,
		r: (m) => (m[1] ? 'а ' : '') + (m[2] ? 'я ' : '') + 'нич' + (m[3].match(/то/i) && !m[5] ? 'его' : m[3].toLowerCase()) + (m[5] ? ' ' + (m[6] == 'как' ? 'ни' : '') + m[6].toLowerCase() + '.' : ' (:'),
	},
