...
function dateStr(d) {
	...
	return d.toJSON().split(".")[0].replace(/T/, ' ');
}
...
var responses = [
	...
	// nastey words
	{
		p: /((^|[^а-яё])(([саоув]|(пр|[дзвпн])[аеёиыяюе]|(р[ао]|в)[сз]|[оа]т)?(ху[йеяюиё]|п[иеёюй]з[жд]|др(оч|ач(?!л))|п[ие]д[ои](?!н))|[её]б|муд[аеёиыяюо]|[бм]л(я|э(?![кс]))|[её]пт)|[ьъоаеёу][её]б|(fu|di)ck)/i,
		r: 'please, be polite!',
	},
	// dirtey words
	{
		...
		+ ['', '+png', '+photo', '+screenshot', '+cute', '+dance', '+jpg'].pick(), response => {
			...
			response.on('end', () => {
				...
				*/
				...
				resolve({text: 'держи:', files: [{attachment: url}]});
			});
	...
	{
		d: true,
		p: /(^|[^а-яё])у тебя есть (девушк|жен|подружк)[аиы][?! ]*$/i,
		m: 'say',
	...
	{
		d: true,
		p: /(^|[^а-яё])кто( такой)? (тебя созда(ва)?л|созда(ва)?л тебя|тво[йи] создател[ьи])[?! ]*$/i,
		r: 'меня создал сервер в виде абстрактной модели в виртуальной памяти.',
	},
	// автор?
	{
		d: true,
		p: /(^|[^а-яё])кто( такой)? (тебя написал|написал тебя|тво[йи] а(в|фф?)т[оа]ры?)[?! ]*$/i,
	...
	// не о чем разговаривать
	{
		...
		r: 'разве сложно придумать тему для разговора?',
	},
	...
	// как долго онлайн
	{
		...
		r: () => {
			...
			+ 'Время на моих часах при запуске: ' + dateStr(since) + '.\n'
			+ 'Время на моих часах сейчас: ' + dateStr(now) + '.';
		},
	},
	...
	// кто такой кто-то
	{
		...
		r: (m, flags, floodey, message) => {
			let known = {
				...
				'сакуя|идзаёи': 'Сакуя Идзаёи — горничная в поместье Алой Дьяволицы.',
				...
				'(баг|ошибка|bug)': 'баг — ошибка в коде программы, допущенная программистом.',
