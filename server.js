...
var responses = [
	...
	
	{
		...
		r: (
			...
			Function(m, flags, floodey, message, mentioned),
	...
	*/
	...
	// dirtey words
	{
		p: /(^|[^а-яё])((по|на|за|вы|у|про)?(си?ра[тлчкшюей]|ср[иуё])|д[еёи]рьм|г[оа]ве?н|ж[оеё]п|ан(ус|ал(?![ио]))|(на|за|вы|об|раз)?бл[её]в|(вы|[нз]а|обо|про)?сс(?!ы[лпх]|у([чдж]|ту)|а[дж])[ыау]|shit)/i,
		m: 'react',
		r: '🚽',
	},
	...
	{
		d: true,
		p: /^ *((сколько|какое) (сейчас )?)?(текущее )?врем(я|ени)( сейчас)?( у тебя)?( на( твоих)? часах)?[!?., ]*$/i,
		r: () => 'текущее время на моих часах:\n`' + dateStr(new Date()) + '`.',
	},
	...
	// кто такой кто-то
	{
		...
		r: (m, flags, floodey, message) => {
			let known = {
				'(руль?т|rult)': () => customReact(message, 'snetus'),
...
function checkReply(message, flags) {
	...
	for (let item of responses) {
		...
		// custom react
		if (source.m == 'creact') {
			resp = customReact(message, resp);
			// resp will be boolean now
		}
