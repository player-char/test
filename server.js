...
let known = {
	...
	'спам|spam': (m) => {
		...
		while (mail.length < 950) {
			mail += chance(0.8) ? (chance(0.8) ? ' ' : '- ,'.spick()) + spam : '. ? !'.spick() + (chance(0.2) ? '\n\n \n'.spick() : ' ') + slam;
		}
		return ['d', mail + '!'];
	},
	...
	//'(игрок|пл[еао]ер|player)[ _]?(чар|char)': 'PLAYER_CHAR — .',
...
let responses = [
	...
	*/
	...
	// что?
	{
		...
		r: (m) => (m[1] ? 'а ' : '') + (m[2] ? 'я ' : '') + (m[3] ? m[3].toLowerCase() : '') + 'нич' + (m[4].match(/то/i) && m[7] != 'как' ? 'его' : m[4].toLowerCase()) + (m[7] ? ' ' + (m[7] == 'как' ? 'ни' : '') + m[7].toLowerCase() + '.' : ' (:'),
	},
