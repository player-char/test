...
// поиск серверного эмоджи по названию
...
const known = {
	...
	'(игрок|пл[еао]ер|player)[ _-]?(чар|char)': 'PLAYER_CHAR — игрок, которому уже надоело играть в игры.',
...
const responses = [
	...
	
	{
		...
		d?: (
			...
			[ reply even if shy mode is enabled ]
			'force',
	...
	*/
	...
	{
		p: /(^|[^а-яё])((по|на|за|вы|у|про)?(си?ра[тлчкшюей]|ср[иуё](?![лб]))|д[еёи]рьм|г[оа]ве?н|ж[оеё]п|ан(ус|ал(?![ио]))|(на|за|вы|об|раз)?бл[её]в|(вы|[нз]а|обо|про)?сс(?!ы[лпх]|у([чдж]|ту)|а[дж])[ыау]|shit)/i,
		r: ['+', '🚽 ⚠'],
	},
	...
	// dank lexics
	{
		p: /(^|[^а-яёa-z])((олд|нью)?фаг(гот)?([ыиу]|ов|а(ми)?)?)([^а-яёa-z]|$)/i,
		r: ['+', '🐓'],
	},
	...
	// dank words
	{
		p: /(^|[^а-яё])(ч[ёео]-?нить|пели(л|ть)|[ыъь]{2,}|афф|з*а+з+а+з+|а+х+а+х+|л[ыа]л|сми[щш]+но|драт+ут|пей(сат|ш[иеы]т)|пр[еюяё]й?ве[тд]|прифф?(е[тд]|к)|прекол|у?збаг|ниасил|миня|ата(ш[оеё]л|йд)|ухади|(нит|ды|[мт]я|[мтв]и|[чшки]та|(те([бп]е)?|мн[еи]|[еи](й|му?)|нам) плоха)([^а-яё]|$))|^[^а-яёa-z'"]*ы[^а-яёa-z'"]*$/i,
		r: ['+', 'joypain 💉'],
	},
	...
	{
		d: true,
		p: /(Dragon2488|Archengius|AntiquiAvium)/,
		r: (m) => '`' + m[0] + '` is deprecated. Use `' + (m[0] == 'AntiquiAvium' ? 'Archengius' : 'AntiquiAvium') + '` instead.',
	},
	
	// как так?
	{
		p: /^ ?((н[уо]|а|и|только|вот|так(-то)?) )*как(-то)?( (же|вот|так(-то)?))* *[?!]+$/i,
		r: () => [
			'ну вот как-то так.',
	...
	*/
	...
	// не о чем разговаривать
	{
		...
		r: 'ну так придумай тему, разве это так трудно?',
	},
	...
	// cutoff
	{
		p: /(^|[^а-яё])(от|у|за)ре(?!з(ок|к[аиуое]))[жз]/i,
		r: ['+', '✂'],
	},
	...
	{
		d: true,
		p: /(^|[^а-яё])(хочешь|давай|клёв(ое?|ы[йе]?|ая?))([^а-яё]|$)/i,
		c: 0.85,
		r: ['+', '🎣'],
	},
	...
	{
		p: /(^|[^а-яё])(creep(ey|ie|ah)|крипуш(ек|к([аиеу]|ом))?)([^а-яё]|$)/i,
		c: 0.95,
		r: ['+', '🤔'],
	},
	...
	// :creeper:
	{
		p: /(^|[^а-яё])(creep(?!er)|(?!крип(то|(ер([аыуе]|[оа]м|а(х|ми))?|уш(к[ауие]|ек))(?![а-яё])))(от?|[нзвпд][аыое]|(и|ра)[сз]|пе?ре|недо)?крип)/i,
		r: ['+', 'creeper'],
	},
	...
	{
		d: true,
		p: /(^|[^а-яёa-z])((по)?чему|(от)?чего|как(о([йе]|го)|ая|и(е|ми))?|[чк]то)(?![а-яёa-z])[^?]*[?]+/i,
		r: ['+', '🤷'],
	},
